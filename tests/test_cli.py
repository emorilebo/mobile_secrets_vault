...
class TestCLI:
    ...
    def test_set_and_get_secret(self, runner, temp_dir):
        ...
        # Set a secret
        result = runner.invoke(cli, [
            '--vault-file', str(temp_dir / 'secrets.yaml'),
            '--master-key-file', str(temp_dir / 'master.key'),
    ...
    def test_list_versions(self, runner, temp_dir):
        ...
        for i in range(3):
            runner.invoke(cli, [
                '--vault-file', str(temp_dir / 'secrets.yaml'),
                '--master-key-file', str(temp_dir / 'master.key'),
                'set', 'VERSIONED', f'value-{i}'
            ])
        
        result = runner.invoke(cli, [
            '--vault-file', str(temp_dir / 'secrets.yaml'),
            '--master-key-file', str(temp_dir / 'master.key'),
            'list-versions', 'VERSIONED'
        ])
    ...
    def test_rotate_command(self, runner, temp_dir):
        ...
        result = runner.invoke(cli, [
            ...
            '--master-key-file', str(temp_dir / 'master.key'),
            'rotate',
            '--new-key-file', str(temp_dir / 'new_master.key'),
            '--yes'
        ])
